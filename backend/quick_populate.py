#!/usr/bin/env python
"""Quick data population script - bypasses URL loading"""
import os
import sys
import django
...
# Configure Django without checking URLs
django.setup()
...
def fetch_missing_prices():
    ...
    if not missing_tickers:
        print("[OK] All tickers have price data")
        return
                ...
                try:
                    if len(batch) == 1:
                        ...
                        if ticker not in data.columns.get_level_values('Ticker').unique():
                            print(f"    Skip: {ticker} not in yfinance")
                            continue
                    ...
                    if saved > 0:
                        successful += 1
                        print(f"    [OK] {ticker}: {saved} prices")
                    else:
                        failed += 1
    ...
    print(f"[OK] Fetched prices for {successful} tickers, {failed} failed")


def create_stock_analytics():
    ...
    if not missing_tickers:
        print("[OK] All stocks have analytics")
        return
    
    print(f"Creating analytics for {len(missing_tickers)} stocks...")
    ...
    for ticker in missing_tickers:
        try:
            ...
            created += 1
            if created % 50 == 0:
                print(f"  Created {created} analytics...")
        
        except Exception as e:
            failed += 1
            logger.error(f"Error creating analytics for {ticker}: {e}")
    
    print(f"[OK] Created {created} analytics, {failed} failed")
